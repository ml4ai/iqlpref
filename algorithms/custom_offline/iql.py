...
# A query_length > 1 denotes that the r_model is a preference transformer
def qlearning_dataset(
    dataset: minari.MinariDataset, r_model, query_length=1
) -> Dict[str, np.ndarray]:
    obs, next_obs, actions, rewards, dones = [], [], [], [], []
    if query_length > 1:
        for episode in dataset:
            if episode.actions.shape[0] <= query_length:
                sts = episode.observations[:-1][:query_length].reshape(
                    1, -1, episode.observations.shape[1]
                )
                acts = episode.actions[:query_length].reshape(
                    1, -1, episode.actions.shape[1]
                )
                ts = np.arange(episode.actions.shape[0]).reshape(1, -1)
                am = np.ones(episode.actions.shape[0]).reshape(1, -1)
                rwds, _ = r_model(sts, acts, ts, am, training=False)
                obs.append(episode.observations[:-1].astype(np.float32))
                next_obs.append(episode.observations[1:].astype(np.float32))
                rewards.append(rwds["value"].reshape(episode.actions.shape[0]))
                actions.append(episode.actions.astype(np.float32))
                dones.append(episode.terminations)
            else:
                rwds = np.zeros(episode.actions.shape[0])
                sts = episode.observations[:-1][:query_length].reshape(
                    1, query_length, -1
                )
                acts = episode.actions[:query_length].reshape(1, query_length, -1)
                ts = np.arange(query_length).reshape(1, -1)
                am = np.ones(query_length).reshape(1, -1)
                preds, _ = r_model(sts, acts, ts, am, training=False)
                rwds[:query_length] = preds["value"].reshape(query_length)
                for i in range(query_length, episode.actions.shape[0]):
                    sts = np.append(
                        sts,
                        episode.observations[:-1][i].reshape(1, 1, -1),
                        axis=1,
                    )
                    sts = sts[:, -query_length:, :]

                    acts = np.append(
                        acts,
                        episode.actions[i].reshape(1, 1, -1),
                        axis=1,
                    )
                    acts = acts[:, -query_length:, :]
                    ts = np.arange((i + 1) - query_length, i + 1).reshape(1, -1)
                    am = np.ones(query_length).reshape(1, -1)
                    preds, _ = r_model(sts, acts, ts, am, training=False)
                    rwds[i] = preds["value"].reshape(query_length)[-1]
                obs.append(episode.observations[:-1].astype(np.float32))
                next_obs.append(episode.observations[1:].astype(np.float32))
                rewards.append(rwds)
                actions.append(episode.actions.astype(np.float32))
