#!/usr/bin/env bash
# Launch pt_sweeps as W&B grid sweeps across a set of GPUs.
#
# Run from the REPO ROOT.
#
# Usage:
#   ./pt_sweeps/launch.sh [SWEEP] [GPU_LIST] [AGENTS_PER_GPU]
#
#   SWEEP           "all" (the 4 pt sweeps: one per antmaze variant) or a single sweep: full path,
...
if [[ ! -d pt_sweeps || ! -f algorithms/offline/iql.py ]]; then
  echo "ERROR: run this from the iqlpref repo root:  ./pt_sweeps/launch.sh" >&2
  exit 1
fi
...
CPU_PER_RUN=25
LOGDIR="pt_sweeps/logs"; mkdir -p "$LOGDIR"

# --- resolve sweep yaml(s) ---
SWEEP_YAMLS=()
if [[ "$SWEEP_ARG" == "all" ]]; then
  SWEEP_YAMLS=(
    pt_sweeps/sweep_antmaze_medium_play.yaml
    pt_sweeps/sweep_antmaze_medium_diverse.yaml
    pt_sweeps/sweep_antmaze_large_play.yaml
    pt_sweeps/sweep_antmaze_large_diverse.yaml
  )
else
  y="$SWEEP_ARG"
  [[ "$y" == *.yaml ]] || y="${y}.yaml"
