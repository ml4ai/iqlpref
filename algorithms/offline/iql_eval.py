...
#
# iql_eval.py — seed-tied reward-model variant of iql.py.
#
# Identical to iql.py except that the reward-model directory is derived from the
# training ``seed`` instead of being a fixed path.  The antmaze eval reward-model
# training scripts (run_{bnn,mr,pt}_training_antmaze_eval.py) write each run to a
# deterministic, per-seed directory ``{OUT_DIR}_{seed}``.  Here you pass that
# ``{OUT_DIR}`` base as ``reward_model_root`` and iql_eval appends ``_{seed}``,
# guaranteeing the IQL global seed and the reward model it evaluates always match.
#
# Set ``reward_model_root`` (not ``reward_model_path``) in the sweeps.  If both are
# given, ``reward_model_root`` wins.  All other behaviour is unchanged from iql.py.
import copy
import os
import random
import re
...
class TrainConfig:
    ...
    # Base output dir of the per-seed reward-model training run, WITHOUT the
    # trailing ``_{seed}`` (e.g. ".../exp/reward_learning/antmaze_medium_play_bnn_eval").
    # When set, the actual reward_model_path is derived in __post_init__ as
    # ``{reward_model_root}_{seed}`` so the IQL seed and the reward model always
    # match.  Takes precedence over reward_model_path when both are given.
    ...
    def __post_init__(self):
        ...
        # Tie the reward-model directory to the training seed: the eval training
        # scripts write each run to a deterministic "{root}_{seed}" directory.
        if self.reward_model_root:
            self.reward_model_path = f"{self.reward_model_root}_{self.seed}"
