...
class TrainConfig:
    ...
    reward_type: int = (
        0  # 0: posterior predictions, 1: posterior predictive mean, 2: posterior predictive median, 3: posterior predictions given map
    )
...
def qlearning_dataset(
    dataset: minari.MinariDataset, r_model, reward_type, n_samples=None
) -> Dict[str, np.ndarray]:
    obs, next_obs, actions, rewards, dones = [], [], [], [], []
    if reward_type == 1:
        assert n_samples is not None
        for episode in dataset:
            _, _, r_preds = r_model.predict(
...
def train(config: TrainConfig):
    ...
    qdataset = qlearning_dataset(
        dataset, bayes_net, config.reward_type, config.n_samples
    )
